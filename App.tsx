...
export default function App() {
  ...
  const handleDownload = (task: Task) => {
    ...
    addLog(AgentRole.EXECUTION, `Downloaded document as Word (.doc): ${task.outputType || 'DOC'}_${task.id}.doc`, 'ACTION');
  };
  ...
  const processTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      if (!effectiveKey) {
         ...
         let mockType = TaskType.INFORMATIONAL;
         if (task.rawContent.toLowerCase().includes('prd') || task.rawContent.toLowerCase().includes('feature')) {
           mockType = TaskType.ACTION_PRD;
         } else if (task.rawContent.toLowerCase().includes('email') || task.rawContent.toLowerCase().includes('reply')) {
           mockType = TaskType.ACTION_EMAIL;
         } else if (/\d|SLA|policy|limit|spec/i.test(task.rawContent)) {
           mockType = TaskType.ACTION_KB;
         }
         ...
         } else if (decision.outputType === 'ACTION_EMAIL') {
         ...
         } else if (decision.outputType === 'ACTION_KB') {
            output = `## Knowledge Base Update (Simulated)\n\n**Change Detected**: ${task.summary}\n\n**Details**: This is a simulated KB entry. In real mode, Gemini would extract the specific policy or number change here.`;
         } else {
            output = undefined;
         }
         addLog(AgentRole.EXECUTION, output ? "Content generated successfully (Simulated)" : "No action taken (Simulated)", 'RESULT', { preview: output ? "..." : "N/A" });
              ...
              <div key={doc.id} className="bg-ops-card border border-ops-border rounded-xl p-6 hover:shadow-lg transition-all group flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div className={`p-2 rounded-lg ${
                    doc.outputType === 'ACTION_PRD' ? 'bg-purple-500/10 text-purple-500' : 
                    doc.outputType === 'ACTION_EMAIL' ? 'bg-blue-500/10 text-blue-500' : 
                    doc.outputType === 'ACTION_KB' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-500/10 text-ops-text'
                  }`}>
                    <FileText size={24} />
                  </div>
